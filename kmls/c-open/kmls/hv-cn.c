...
klee_make_symbolic( &sym_ee_len_cn, sizeof(uint8_t), "sym_ee_len_cn" );
    ...
    klee_assume( sym_ee_len_cn <= 0x04 ); 
...
// klee_assume( sym_host_in_len == 0x01 );
    ...
    // klee_assume( sym_host_in_len <= 0x02 );
    // klee_assume( sym_host_in_len >= 0x01 );
#else
...
// string type symbol
    ...
    memset( &ee_cert[176], type, 1 );
    // uint8_t str_type_set = {12, 18, 20, 21, 22, 25, 26, 27, 28, 30};
    // the `type` should be limited to the set of string types
    // klee_assume( type == 12 || type == 18 || type == 19 || type == 20 || type == 21 || type == 22 || type == 25 || type == 26 || type == 27 || type == 28 || type == 30 );
    // type should be in [0, 30]
...
// create name symbol
	...
	* ---------------------------------------------------------- */
    ...
    if(!cert){
        printf("RETURN: Error loading cert into memory\n");
        return 1;
    }
