...
#define VERIFY 1
#define TEST_CODE -2
#define PRINT_DEBUG 0 // this is for debugging purpose
...
klee_make_symbolic( &sym_ca_len_cn, sizeof(uint8_t), "sym_ca_len_cn" );
    ...
    klee_assume( sym_ca_len_cn <= 0x05 ); // here we assume the cn length is less than 0x10
    klee_assume( sym_ca_len_cn >= 0x01 );
#else
    klee_assume( sym_ca_len_cn == 0x04 ); // concrete value
#endif // LEN_SYMBOLIC
...
const char* name = NULL;
    ...
    ret = mbedtls_x509_crt_parse_der(mytrusted_chain_ptr, ca_cert, sizeof(ca_cert));
    if(ret != 0) return -1;
    ret = mbedtls_x509_crt_parse_der(mychain_ptr, ee_cert, sizeof(ee_cert));
    if(ret != 0) return -1;
