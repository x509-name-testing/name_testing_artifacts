...
klee_make_symbolic( &sym_ee_len_hn, sizeof(uint8_t), "sym_ee_len_hn" );
    ...
    klee_assume( sym_ee_len_hn <= 0x05 ); // here we assume the cn length is less than 0x10
    klee_assume( sym_ee_len_hn >= 0x01 );
#else
...
// create name symbol
    ...
    name[real_len_host_in] = 0;
    // uint8_t sym_name[real_len_host_in];
    // uint8_t* name = (uint8_t*) malloc(real_len_host_in * sizeof(uint8_t));
    memset( name, 0xff, real_len_host_in );
    klee_make_symbolic( name, sizeof(name), "name" );
    // klee_make_symbolic( sym_name, sizeof(sym_name), "sym_name" );
    // memcpy( name, sym_name, sizeof(sym_name) );    
// end of name symbol
...
// end of name constraints
    ...
    ret = mbedtls_x509_crt_parse_der(mytrusted_chain_ptr, ca, sizeof(ca));
    if(ret != 0) return -1;
    ret = mbedtls_x509_crt_parse_der(mychain_ptr, ee_cert, len_ee_cert);
    if(ret != 0) return -1;

    uint32_t verify_results = 0;
    // name chaining + hostname verification
    ret = mbedtls_x509_crt_verify(mychain_ptr, mytrusted_chain_ptr, NULL, name, &verify_results, NULL, NULL);

    // printf("verify_results = %u\n", verify_results);
    if (verify_results == 0){
        printf("ok\n");
        return 0;
    } else {
        printf("err with ret code / verf code: %d / %d\n", ret, verify_results);
        return 1;
    }
#else
    return TEST_CODE;
