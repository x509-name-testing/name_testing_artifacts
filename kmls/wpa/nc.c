...
0x11, 0x04, 0x08, 0x30, 0x06, 0x82, 0x04, 0x62, 0x62, 0x62, 0x62, 0x30, 0x1D, 0x06, 0x03, 0x55, 
...
/* The NULL is not allowed in wpa, so it should be negated in every bytes.*/
  for ( int i = 0; i < real_len_ca; i++ ) 
    klee_assume( sym_ca_cn[i] != 0x00 );

  for ( int i = 0; i < real_len_ee; i++ ) 
    klee_assume( sym_ee_cn[i] != 0x00 );


#if VERIFY

  struct x509_certificate *ca = NULL;
  struct x509_certificate *ee = NULL;

  ca = x509_certificate_parse(ca_cert, sizeof(ca_cert));
  ...
  ee = x509_certificate_parse(ee_cert, sizeof(ee_cert));

  if ( ee == NULL ) {
      printf("Failed to parse EE certificate\n");
      return 1;
  } 
  struct x509_certificate *trusted = ca;
  struct x509_certificate *chain = ee;

  int ret_val;
  int ret = x509_certificate_chain_validate( trusted, chain, &ret_val, 0);
  int res = 0;
  printf("ret: %d %d\n", ret, ret_val);
  printf("\nRETURN: ");
  if (ret == 0) {
      ...
      res = 1;
      printf("Certificate chain is invalid: err code %d\n", ret_val);
  }

  x509_certificate_chain_free(trusted);
