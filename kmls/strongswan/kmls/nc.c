...
// // end of name constraints
    ...
    library_init("emptyconf", "verify");
    if (! lib->plugins->load(lib->plugins, "aes des rc2 sha2 sha1 md5 mgf1 random x509 revocation pubkey pkcs1 pkcs7 pkcs12 dnskey sshkey pem pkcs8 gmp curve25519 hmac kdf drbg") ) {
	...
	ca = lib->creds->create(lib->creds, CRED_CERTIFICATE, CERT_X509,
                            BUILD_BLOB_ASN1_DER, chunk_create(ca_cert, sizeof(ca_cert)), BUILD_END);
	if (!ca){
		fprintf(stderr, "parsing ca failed\n");
		return FALSE;
	}
    ...
    // printf("loading ee cert\n");
    ee = lib->creds->create(lib->creds, CRED_CERTIFICATE, CERT_X509,
							BUILD_BLOB_ASN1_DER, chunk_create(ee_cert, sizeof(ee_cert)), BUILD_END);
    if (!ee) {
        printf("error loading ee cert\n");
        return 1;
    }
    ee = creds->add_cert_ref(creds, !has_ca, ee);

    enumerator = lib->credmgr->create_trusted_enumerator(lib->credmgr,
									KEY_ANY, ee->get_subject(ee), FALSE);


    printf("\nRETURN: ");
    if (enumerator->enumerate(enumerator, &ee, &auth)) {
        enumerator->destroy(enumerator);
        printf("ok\n");
        ee->destroy(ee); ca->destroy(ca);
        return 0;
    } else {
