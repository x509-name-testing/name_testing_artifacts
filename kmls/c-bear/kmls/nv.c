...
static size_t my_read_trust_anchors(anchor_list *dst, const unsigned char * ca_buf, size_t len) {
	br_x509_certificate xcs;
	anchor_list tas = VEC_INIT;
	size_t u, num;

//	xcs = read_cert(ca_buf, len);
//	if (xcs == NULL) {
//		return 0;
//	}
	...
	for (u = 0; u < num; u ++) {
		...
		ta = certificate_to_trust_anchor(&xcs);
	...
	VEC_ADDMANY(*dst, &VEC_ELT(tas, 0), 1);
	// VEC_CLEAR(tas);
	// free_certificates(xcs, num);
	return num;
}
...
// uint8_t sym_sth0[4];
// klee_make_symbolic( sym_sth0, sizeof(sym_sth0), "sym_sth0" );
// memcpy( &ca_cert[94], sym_sth0, sizeof(sym_sth0) );
...
ACCU_SYMVAR( sym_host_in_len, real_len_host_in );
    ...
    // memset( name, 0xff, real_len_host_in + 1 );
    klee_make_symbolic( name, sizeof(name), "name" );
    klee_assume( name[real_len_host_in] == 0);
	...
	size_t caLen = sizeof(ca_cert);
	size_t certLen = sizeof(ee_cert);
	...
	if (my_read_trust_anchors(&anchors, ca_cert, caLen) == 0) {
		return -1;		
	}

    cert.data = ee_cert;
    cert.data_len = certLen;

	VEC_ADDMANY(chain, &cert, 1);
